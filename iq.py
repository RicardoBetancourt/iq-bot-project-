...
class IQOptionBot:
    ...
    def setup_ui(self):
        ...
        # Gráficos (imágenes generadas dinámicamente)
        self.chart_img = ft.Image(src="", width=780, height=338, fit=ft.ImageFit.CONTAIN, visible=False)
        self.bar_img = ft.Image(src="", width=390, height=234, fit=ft.ImageFit.CONTAIN, visible=False)
        
        # Diseño principal
        self.page.add(
            ft.Column([
                ft.Row([self.email_field, self.password_field]),
    ...
    def train_model(self, df):
        ...
        self.update_log(f"Modelo entrenado con precisión: {accuracy:.2%}")
...
class BotFletApp:
    def __init__(self, page: ft.Page):
        ...
        self.page.bgcolor = "#B0B3B8"
        self.bot_thread = None
        self.bot = None
        self.running = False
        self.logs = ft.ListView(height=180, spacing=2, auto_scroll=False)
        self.chart_img = ft.Image(src="", width=780, height=338, fit=ft.ImageFit.CONTAIN, visible=False)
        self.bar_img = ft.Image(src="", width=390, height=234, fit=ft.ImageFit.CONTAIN, visible=False)
        isa_text = "#111"
        card_gray = "#F2F3F4"
        button_gray = "#888A8D"
        button_gray_hover = "#6D6F73"
        button_gray_disabled = "#D1D3D4"
        self.percent_card = ft.Container(
            content=ft.Text("% PYG: --", size=22, weight=ft.FontWeight.BOLD, color=isa_text),
            bgcolor=card_gray,
            border_radius=10,
            padding=15,
        ...
        self.pyg_usd_card = ft.Container(
            content=ft.Text("PYG: -- USD", size=22, weight=ft.FontWeight.BOLD, color=isa_text),
            bgcolor=card_gray,
            border_radius=10,
            padding=15,
            alignment=ft.alignment.center,
        ...
        # --- NUEVO: Dropdown para tipo de cuenta ---
        ...
        self.page.add(
            ft.Column([
                ...
                # --- Cambia aquí: agrega el dropdown junto a email y password ---
                ft.Row([self.email_input, self.password_input, self.account_type], alignment=ft.MainAxisAlignment.CENTER),
                ft.Divider(),
                ...
                ft.Divider(),
                ft.Row([
                    ft.Text("Rentabilidad del Bot", size=18, weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.CENTER)
                ], alignment=ft.MainAxisAlignment.CENTER),
                ft.Row([
                    self.chart_img,
                    self.bar_img,
    ...
    def mostrar_grafico_rentabilidad(self):
        ...
        try:
            ...
            if df_balance.shape[0] >= 1:
                # Determinar color de la línea: verde si ganancia, rojo si pérdida (vs balance inicial)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    marker=dict(color=marker_color, size=8, line=dict(width=2, color='#222')),
                    hovertemplate='<b>Hora:</b> %{x}<br><b>Balance:</b> %{y:.2f} $<extra></extra>'
                ))
                fig.update_layout(
                    title=dict(text='<b>Evolución del Balance</b>', font=dict(size=26, color='#0099FF'), x=0.5),
                    xaxis=dict(
                        title=dict(text='Hora', font=dict(color='black')),
                        showgrid=False, zeroline=False, showline=True, linecolor='#888', tickfont=dict(color='black')
                    ),
                    yaxis=dict(
                        title=dict(text='Balance ($)', font=dict(color='black')),
                        showgrid=False, zeroline=False, showline=True, linecolor='#888', tickfont=dict(color='black')
                    ),
                    ...
                    height=338,
                    margin=dict(l=39, r=39, t=52, b=39),
                    showlegend=False,
                    hovermode='x unified'
                )
                buf = io.BytesIO()
            ...
            if df_ops.shape[0] == 0 and df.shape[0] >= 1:
                # Si no hay cerradas, mostrar la primera como barra neutra
                win_count = 0
                loss_count = 0
            ...
            bar_fig.update_layout(
                title=dict(text='<b>Ganadas vs Perdidas</b>', font=dict(size=23, color='#0099FF'), x=0.5),
                xaxis=dict(
                ...
                barmode='group',
                height=234,
                margin=dict(l=26, r=26, t=52, b=26),
                showlegend=False,
                hovermode='x unified'
            )
            buf2 = io.BytesIO()
            ...
            if df_balance.shape[0] >= 1:
                ...
                self.percent_card.content = ft.Text(f"% PYG: {pct:.2f}%", size=22, weight=ft.FontWeight.BOLD, color=color)
                ...
                self.pyg_usd_card.content = ft.Text(f"PYG: {usd:+.2f} USD", size=22, weight=ft.FontWeight.BOLD, color=usd_color)
    ...
    def start_bot(self, e):
        ...
        self.page.update()
        self.running = True
        self.bot_thread = threading.Thread(target=self.run_bot, args=(email, password), daemon=True)
    ...
    def run_bot(self, email, password):
        try:
            # --- Cambia aquí: pasa el tipo de cuenta seleccionado ---
            account_type = self.account_type.value or "PRACTICE"
